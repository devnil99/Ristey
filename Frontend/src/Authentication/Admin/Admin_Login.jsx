...
function Admin_login() {
  ...
  const handleSubmit = async (values) => {
    try {
      ...
      if (response?.token?.access) {
        ...
        message.error(response?.detail || 'Login failed. Please check your credentials.');
      }
    } catch (error) {
  ...
  return (
    <div className="admin-login-container"> {/* Replaces outer div with inline style */}
      <Card className="admin-login-card"> {/* Replaces Card with inline style */}
        ...
        >
          ...
          >
            {/* width: '90%' will be handled by CSS or AntD's default block behavior with layout="vertical" */}
          ...
          <Form.Item className="login-button-container">
            ...
            <Button type="primary" htmlType="submit" className="login-submit-button" block>
              Login
            </Button>
          </Form.Item>
        </Form>
